...
<div class="content settings">
      ...
      <ul class="list-group">
        ...
        <li class="list-group-item">
          <div class="input-group mb-3">
            ...
            {if $isAdd && !empty($POST)}
              <input type="text" name="username" maxlength="20" class="form-control" required="required"
                placeholder="{t}Benutzername{/t}" aria-label="{t}Benutzername{/t}" aria-describedby="basic-addon1"
                value="{$POST.username}">
            {else if $isEdit}
              <input type="text" name="username" maxlength="20" class="form-control" required="required"
                placeholder="{t}Benutzername{/t}" aria-label="{t}Benutzername{/t}" aria-describedby="basic-addon1"
        ...
        <li class="list-group-item">
          <div class="input-group mb-3">
            ...
            {if $isAdd && !empty($POST)}
              <input type="email" name="mailaddress" maxlength="254" class="form-control" autocomplete="off"
                required="required" placeholder="{t}E-Mail{/t}" aria-label="{t}E-Mail{/t}" aria-describedby="basic-addon7"
                value="{$POST.mailaddress}">
            {else if $isEdit }
              <input type="email" name="mailaddress" maxlength="254" class="form-control" autocomplete="off"
                required="required" placeholder="{t}E-Mail{/t}" aria-label="{t}E-Mail{/t}" aria-describedby="basic-addon7"
        ...
        <li class="list-group-item">
          <div class="input-group mb-3">
            <div class="input-group-prepend">
              <div class="dropdown">
                <select class="btn dropdown-toggle" tabindex="-1" autocomplete="off" type="button" id="usergroupDropdown"
            ...
            {if $isAdd && !empty($POST)}
              <input type="text" class="form-control" id="usergroupname" name="usergroupname" readonly="readonly"
                required="required" autocomplete="off" placeholder="{t}Benutzergruppe{/t}" value="{$POST.usergroupname}">
              <input type="hidden" value="{$POST.usergroupid}" id="usergroupid" name="usergroupid" />
            {else if $isEdit}
              <input type="text" class="form-control" id="usergroupname" name="usergroupname" readonly="readonly"
                required="required" autocomplete="off" placeholder="{t}Benutzergruppe{/t}" value="{$user.usergroupname}">
              <input type="hidden" value="{$user.usergroupid}" id="usergroupid" name="usergroupid" />
            {else}
              <input type="text" class="form-control" id="usergroupname" name="usergroupname" readonly="readonly"
                required="required" autocomplete="off" placeholder="{t}Benutzergruppe{/t}">
        ...
        <li class="list-group-item">
          <div class="input-group mb-3">
            <div class="input-group">
              <div class="input-group-prepend">
                <div class="dropdown">
                  <select class="btn dropdown-toggle" tabindex="-1" autocomplete="off" type="button" id="userapiDropdown"
                    data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
              ...
              {if $isAdd && !empty($POST)}
                <input type="text" class="form-control" id="userapi" name="userapi" readonly="readonly" required="required"
                  autocomplete="off" placeholder="{t}API Zugriff{/t} "
                  value="{if $POST.userapikey == 1}{t}erlauben{/t}{else}{t}verbieten{/t}{/if}">
                <input type="hidden" value="{$POST.userapikey}" id="userapikey" name="userapikey" />
              {else if $isEdit}
                <input type="text" class="form-control" id="userapi" name="userapi" readonly="readonly" required="required"
                  autocomplete="off" placeholder="{t}API Zugriff{/t} "
                  value="{if $user.api_access == 1}{t}erlauben{/t}{else}{t}verbieten{/t}{/if}">
  ...
  {else}
    ...
    {if $update_available}
      <div class="alert alert-danger" role="alert">
        <h6></i><a href='install.php'><i class="fa fa-sync"></i>{t}Bitte die Datenbank aktualisieren{/t}<i
              class="fa fa-sync"></i></a></h6>
      </div>
    {/if}

    {if $useRegistration}
      <ul class="categories userlist list-group">
        <li class="alert alert-info"><span class="list-span">{t}Benutzername{/t}</span><span
            class="list-span">{t}E-Mail{/t}</span><span class="list-span">{t}Gruppe{/t}</span><span class="list-span">{t}API
            Zugriff{/t}</span><span class="list-span">{t}Aktionen{/t}</span></li>
        {foreach $users as $user}
          <li class="list-group-item"><a title="{t}Benutzer löschen{/t}" name="removeUser"
              href="{$urlBase}/settings{$urlPostFix}?removeUser={$user.id}" class="removalButton btn"><i
                class="fas fa-times-circle"></i></a><span class="list-span">{$user.username}</span><span
              class="list-span">{$user.mailaddress}</span><span class="list-span">{$user.usergroupname}</span><span
              class="list-span">{if !isset($user.api_access)}{t}Bitte die Datenbank
                aktualisieren{/t}
                {else}
                  {if $user.api_access == 1}<i class="fas fa-circle-check"></i>
                  {else}<i
                    class="fas fa-ban"></i>{/if}
                  {/if}</span><a class="fas fa-edit editUser" href="#" name="editUser"
                data-name="{$user.username}" data-id="{$user.id}"></a></li>
          {/foreach}
          <li class="list-group-item">
            <a title="{t}Benutzer anlegen{/t}" name="addUser" href="{$urlBase}/settings{$urlPostFix}?addUser"
              class="btn btn-primary float-right">{t}Benutzer anlegen{/t}</a>
          </li>
        </ul>
        ...
        <form accept-charset="utf-8" id="mailform" method="POST" action="">
          ...
          <ul class="categories list-group">
            <li class="alert alert-info">
              <span class="list-span">{t}E-Mailserver-Einstellungen{/t}&nbsp;-&nbsp;{if $mailSettings.enabled}{t}Status:
                Aktiviert{/t}
                {else}
                  {t}Status: Deaktiviert
                  {/t}
                {/if}</span>
            </li>
            <li class="list-group-item">
              <div class="input-group">
                ...
                <input type="email" name="senderAddress" maxlength="254" class="form-control" autocomplete="off"
                  placeholder="email@example.com" aria-label="Absender" aria-describedby="basic-addon7"
              ...
              <div class="input-group">
                ...
                <div class="form-check form-check-inline">
                  <input class="form-check-input" type="radio" name="mail_enabled" id="mail_enabled_off" value="false"
                    {if $mailSettings.enabled}checked="checked" {/if}>
                  <label class="form-check-label" for="mail_enabled_off">{t}deaktivieren{/t}</label>
                </div>
                <div class="form-check form-check-inline">
                  <input class="form-check-input" type="radio" name="mail_enabled" id="mail_enabled_on" value="true"
                    {if !$mailSettings.enabled}checked="checked" {/if}>
      ...
      <form accept-charset="utf-8" id="updateForm" method="POST" action="">
        ...
        <ul class="categories list-group">
          <li class="alert alert-info">
            <span class="list-span">{t}Installation und Aktualisierung{/t}&nbsp;-&nbsp;{if $install_allowed}{t}Status:
              Aktiviert{/t}
              {else}
                {t}Status: Deaktiviert
                {/t}
          ...
          <li class="list-group-item">
            <div class="input-group">
              ...
              <div class="form-check form-check-inline">
                <input class="form-check-input" type="radio" name="allow_install" id="install_off" value="deny"
                  {if $install_allowed}checked="checked" {/if}>
                <label class="form-check-label" for="install_off">{t}verbieten{/t}</label>
              </div>
              <div class="form-check form-check-inline">
                <input class="form-check-input" type="radio" name="allow_install" id="install_on" value="allow"
                  {if !$install_allowed}checked="checked" {/if}>
          ...
          <li class="list-group-item">
            <div class="input-group">
              ...
              <input type="text" class="form-control" id="startpagename" name="startpagename" readonly="readonly"
                required="required" autocomplete="off" value="{$pages.$defaultStartPage}" placeholder="{t}Startseite{/t}">
        ...
        <ul class="list-group">
          <li class="alert alert-info">
            ...
            <small>
              ...
              <br><span>{t}Betatest{/t} - {t}Noch nich veröffentlichte Funktionen, aber möglicherweise auch mit
                Fehlern{/t}</span>
              <br><span>{t}Entwicklung{/t} - {t}Aktueller Stand der Entwicklung - mit Vorsicht zu geniesen - Fehler
                eingeschlossen{/t}</span>
            </small>
          </li>
          <li class="list-group-item">
            <div class="input-group">
              <div class="input-group-prepend">
                <div class="dropdown">
                  <select class="btn dropdown-toggle" tabindex="-1" autocomplete="off" type="button" id="branchDropdown"
                    data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" placeholder="{t}Update-Quelle{/t}">
              ...
              <input type="text" class="form-control" id="branchlabel" name="branchlabel" readonly="readonly"
                required="required" autocomplete="off" placeholder="{t}Version{/t} " value="{$outputlabel}">
              <input type="hidden" value="{$settingdata.updater.githubbranch}" id="branch" name="branch" />
            </div>
            <button id="updaterunlockbutton" class="btn btn-primary float-left"
              onclick="unlockupdater();return false;">{t}Updatequellen-Auswahl aktivieren{/t}</button>
            <button type="submit" id="updaterbutton" class="btn btn-primary float-right" disabled>{t}Einstellungen
              speichern{/t}</button>
          </li>
        </ul>
        <div class="clearfix"></div>
