  ...
  <div class="srcDestContainer">
    ...
    <div>
      <div class="clearfix storageListing storeSrc">
        <h2>{t}Quelle{/t}</h2>
        <div class="data" data-type="src">
          <p>{t}Quelle wählen.{/t}</p>
        </div>
      </div>
      <div class="float-left storageListing storeDestDiv">
        <div class="storeDest">
          <h2>{t}Ziel{/t}</h2>
          <div class="data" data-type="dest">
            <p>{t}Ziel wählen.{/t}</p>
          </div>
        </div>
      ...
      {literal}
        <script type="text/javascript">
          ...
          function transferItem(evt) {
            evt.preventDefault()
            if (evt.target.dataset['sid'] === document.querySelector('.switchStorage[data-type="storeDest"]').value ||
              parseInt(document.querySelector('.switchStorage[data-type="storeDest"]').value) === -1) return
            if (evt.target.parentNode.dataset['type'] === 'src') {
              ...
              if (index === -1) {
                ...
                if (amount < parseInt(srcTarget.dataset['amount'])) srcTarget.textContent = srcTarget.textContent.replace(
                  amount, amount + transferItemAmounts[index], 1)
              ...
              if (index !== -1) {
                ...
                if (srcTarget !== null) {
                  ...
                  let amount = parseInt(srcTarget.textContent.match(/[0-9]*/)[0])
                  if (amount < parseInt(srcTarget.dataset['amount'])) srcTarget.textContent = srcTarget.textContent
                    .replace(amount, amount + transferItemAmounts[index], 1)
                }
          ...
          function transferItems(evt) {
            ...
            request.open('GET', 'transfer.php?transferTarget="' + targetId.toString() + '"&transferIds="' +
              transferItemIds.toString() + '"' + '"&transferAmounts="' + transferItemAmounts.toString() + '"')
            request.send()
