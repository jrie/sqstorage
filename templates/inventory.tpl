      ...
      {foreach $myitem as $itemstore}
        ...
        {if $parse.showemptystorages || $itemstore.itemcount > 0 }
          ...
          <div class="storage-area">
            ...
            <h4 class="text-dark">
              {if !empty($itemstore.storage.label)}<a href="{$urlBase}/inventory{$urlPostFix}?storageid={$itemstore.storage.id}">{if isset($itemstore.storage.label)}{$itemstore.storage.label}{else}{t}Nicht zugewiesen{/t}{/if}</a>{else}{t}Nicht zugewiesen{/t}{/if}&nbsp;{if !$isGuest && !empty($itemstore.storage.label)}
              <a title="{t}Lagerplatz umbenennen{/t}" onclick="changeSingleValue('storages','label',{$itemstore.storage.id},true);" href="javascript:void(0)"><i class="fas fa-edit fa-xs"></i></a>
            {/if}
          ...
          <div class="collapsestorage" id="itemlist_{$itemstore.storage.id}">
            ...
            {if !$isGuest && isset($itemstore.storage.id) && isset($itemstore.storage.label && !empty($itemstore.storage.label)) }
