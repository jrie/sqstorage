  ...
  <script type="text/javascript">
    ...
    function checkSubmitData(evt) {
      ...
      for (let input of inputs) {
        if (input.getAttribute('readonly') !== null || input.getAttribute('hidden') !== null || input.value.trim() === '')
          continue
    ...
    function resetFields() {
      ...
      let visibleInCategories = document.querySelector('select[name="visibleInCategories[]"]').parentNode.children[0]
        .children[2].children
      ...
      for (let field of dataFields) {
        ...
        if (fieldConverts[field.value] !== undefined) {
          if (fieldConverts[field.value] === 'int' || fieldConverts[field.value] === 'float') {
            ...
            if (isNaN(tmpValue) || tmpValue === undefined || tmpValue < validData[dataValue][0] || tmpValue > validData[
                dataValue][1]) {
        ...
        if (nonValidInputs[field.name] !== undefined) {
          if (dataValue === nonValidInputs[field.name][0] && dataValue !== '') {
            ...
            evt.preventDefault()
            break
          } else if (dataValue.length < nonValidInputs[field.name][2] || dataValue.length > nonValidInputs[field.name][
              3
            ]) {
        ...
        for (let value of visibleSelections) {
          ...
          for (let option of options) {
            if (option.getAttribute('value') === value) {
              ...
              document.querySelector('input[name="visibleInCategories_input"]').parentNode.children[2].classList.add(
                'hide')
              document.querySelector('input[name="visibleInCategories_input"]').parentNode.children[2].classList.remove(
                'show')
              break
            }
          }
