...
{include file="nav.tpl" target="categories.php" request=$REQUEST}
    ...
    {$alert}
            ...
            {foreach $subCategories as $category}
                <li class="list-group-item"><a name="removeSubcategory" tabindex="-1" data-name="{$category.name}" href="{$urlBase}/categories?removeSubcategory={$category.id}" class="removalButton fas fa-times-circle btn"></a><a class="list-span" data-name="{$category.name}" href="{$urlBase}/inventory?subcategory={$category.id}">{$category.name}</a><span class="list-span">{$category.amount} {if $category.amount == 1}{t}Gegenstand{/t}{else}{t}Gegenstände{/t}{/if}</span><a class="fas fa-edit editCategory" href="#" name="editSubcategory" data-name="{$category.name}" data-id="{$category.headCategory}"></a>
